...
drivetrain Drivetrain = drivetrain(LeftDriveSmart, RightDriveSmart, 319.19, 381, 355.59999999999997, mm, 1);
motor claw = motor(PORT8, ratio36_1, true);
motor backarm = motor(PORT14, ratio36_1, false);
...
int rc_auto_loop_function_Controller1() {
  ...
  while(true) {
    if(RemoteControlCodeEnabled) {
      ...
      // check the ButtonR1/ButtonR2 status to control frontarms
      if (Controller1.ButtonR1.pressing()) {
        frontarms.spin(reverse);
        Controller1RightShoulderControlMotorsStopped = false;
      } else if (Controller1.ButtonR2.pressing()) {
        frontarms.spin(forward);
        Controller1RightShoulderControlMotorsStopped = false;
